...
class Nilai extends CI_Controller
{
    ...
    public function data_nilai_perkd()
    {
        ...
        if ($data != null || $jenis != null) {
            ...
            $html = $html . '<div class="card">
                    <div class="card-body">
                        ' . anchor('walikelas/nilai/input?id_mapel=' . $id_mapel . '&id_kd=' . $id_kd . '&nilai=' . $nilai, '<button class="btn btn-sm btn-primary mb-3 mr-2"><i class="fas fa-plus fa-sm"></i> Tambah Nilai</button>') . '
                        ' . anchor('walikelas/nilai/archivedata?id_mapel=' . $id_mapel . '&id_kd=' . $id_kd . '&tahun=' . $tahun . '&nilai=' . $nilai, '<button class="btn btn-sm btn-dark mb-3 mr-2"><i class="fas fa-archive fa-sm"></i> Arsip Nilai</button>') . '
                        <h5>' . $kd['nama_kd'] . '</h5>
                        <table class="table table-responsive-sm table-bordered table-striped table-sm w-100 d-block d-md-table">
                            <thead>
                                <tr>
            ...
            foreach ($jenis as $jn => $value) {
                $html = $html . '<th class="text-center">' .
                    anchor('walikelas/nilai/edit?id_mapel=' . $id_mapel . '&id_kd=' . $id_kd . '&jenis=' . $value->jenis . '&nilai=' . $nilai, '<div class="btn btn-sm btn-primary mr-1 ml-1 mb-1"><i class="fa fa-edit fa-sm"></i></div>') .
                    '<a href="' . base_url('walikelas/nilai/archive?id_kelas=' . $id_kelas . '&id_mapel=' . $id_mapel . '&id_kd=' . $id_kd . '&jenis=' . $value->jenis . '&nilai=' . $nilai) . '" class="btn btn-sm btn-dark mr-1 ml-1 mb-1" onclick="return deleteNilai(event)"><i class="fa fa-archive fa-sm"></i></a>' .
                    '</th>';
            }
    ...
    public function edit()
    {
        ...
        if (!isset($id_mapel) || !isset($id_kd) || !isset($jenis) || !isset($nilai)) {
            redirect('error_404');
        }
    ...
    public function archivedata()
    {
        ...
        if (!isset($id_mapel) || !isset($nilai) || !isset($id_kd) || !isset($tahun)) {
            redirect('error_404');
        }
        ...
        $data_nilai     = $this->Arsip_model->get_nilai_perkd($kelas['id_kelas'], $id_mapel, $id_kd, $tahun);
        $jenis          = $this->Arsip_model->get_jenis_nilai_in_perkd($kelas['id_kelas'], $id_mapel, $id_kd, $tahun);
        $kd             = $this->Mapel_model->get_kd_detail($id_kd);
        $result_jenis   = array_column($this->Nilai_model->get_jenis_nilai_in_perkd_array($kelas['id_kelas'], $id_mapel, $id_kd, $tahun), 'jenis');
        $object_jenis   = ['Tugas Harian 1', 'Tugas Harian 2', 'Tugas Harian 3', 'Tugas Harian 4', 'Ulangan Harian 1', 'Ulangan Harian 2', 'Ulangan Harian 3', 'Ulangan Harian 4', 'UTS', 'UAS'];
        ...
        $data = array(
            ...
            'id_mapel'      => $id_mapel,
            'jenis_nilai'   => $nilai,
            'kelas'         => $this->Kelas_model->get_detail_data($kelas['id_kelas']),
            'mapel'         => $this->Mapel_model->get_detail_data($id_mapel),
            'kd'            => $kd,
            'tahun'         => $this->Tahun_model->get_active_stats(),
            'jenis'         => $jenis,
            ...
            'breadcrumb'    => [
                ...
                2 => (object)[
                    'name' => 'Detail',
                    'link' => 'walikelas/nilai/kd?id_kelas=' . $kelas['id_kelas'] . '&id_mapel=' . $id_mapel . '&nilai=' . $nilai
                ],
                3 => (object)[
                    'name' => 'Arsip',
                    'link' => NULL
